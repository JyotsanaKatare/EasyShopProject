...
function VendorChat() {
  ...
  return (
    <div className="h-130 w-full flex flex-col md:flex-row bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      {/* Left Sidebar: Mobile par tab hide hoga jab user select ho */}
      <div className={`${selectedChat ? "hidden md:flex" : "flex"} w-full md:w-1/3 border-r border-slate-100 flex flex-col bg-slate-50/20`}>
        ...
        <div className="overflow-y-auto flex-1">
          {loading && (
            <p className="text-center text-xs text-slate-400 p-4">Loading chats...</p>
          )}
          {!loading && filteredChats.length === 0 && (
            <p className="text-center text-xs text-slate-400 p-4">No chats yet</p>
          )}
          {filteredChats.map((chat) => {
            const chatUser = getChatUser(chat);
            const unread = chat.unreadCount?.get?.(vendorId) || 0;

            return (
              <div
                key={chat._id}
                onClick={() => handleSelectChat(chat)}
                className={`p-4 flex items-center gap-3 cursor-pointer transition-all border-b border-slate-50/50
                          ${selectedChat?._id === chat._id ? "bg-pink-50" : "hover:bg-slate-50"}`}
              >
                <div className="relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex justify-between items-center mt-0.5">
                    <p className="text-[10px] truncate pr-2 text-slate-400 font-medium">
                      {chat.lastMessage?.text || "Click to chat"}
                    </p>
                    {unread > 0 && (
      ...
      {/* Right Side: Mobile par tabhi dikhega jab user selected ho */}
        ...
        {selectedChat ? (
          <>
            <div className="bg-linear-to-br from-pink-500 to-pink-600 p-5 flex items-center gap-3 text-white">
              ...
              <div>
                ...
                <h3 className="text-[10px] md:text-sm font-black uppercase tracking-widest">
                  {getChatUser(selectedChat).name || 'Customer'}
                </h3>
                ...
                messagesList.map((msg, index) => {
                  ...
                  return (
                    <div
                      ...
                      <div className={`max-w-[75%] p-3 rounded-2xl text-[12px] font-bold shadow-sm 
                          ${isMe
            ...
            <div className="p-4 border-t border-slate-50 bg-white flex items-center gap-2">
              ...
              <button
                onClick={sendMessage}
                className="bg-pink-500 text-white p-3 md:px-6 rounded-2xl shadow-lg active:scale-95 flex items-center"
              >
