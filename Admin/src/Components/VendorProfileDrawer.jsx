                ...
                <div className="flex flex-col h-full overflow-hidden">
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-xl border border-slate-100 dark:border-slate-800">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-slate-400 font-medium">Vendor ID: <span className="font-mono">{displayVendor._id?.slice(-8).toUpperCase()}</span></p>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-7 custom-scrollbar">
                        ...
                        <div className="space-y-3">
                            <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                <span className="w-1.5 h-1.5 rounded-full bg-pink-500" /> Contact Information
                            </h4>
                            <div className="grid grid-cols-1 gap-3">
                                {[
                                    { icon: HiOutlineMail, label: 'Email', value: displayVendor.email, color: 'text-blue-500' },
                                    { icon: HiOutlinePhone, label: 'Phone', value: displayVendor.contact || displayVendor.phone || 'N/A', color: 'text-emerald-500' },
                                    { icon: HiOutlineLocationMarker, label: 'Address', value: displayVendor.location || displayVendor.address || 'Not specified', color: 'text-rose-500' },
                                    { icon: HiOutlineCalendar, label: 'Joined On', value: new Date(displayVendor.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }), color: 'text-amber-500' },
                                ].map((item, idx) => (
                        ...
                        <div className="space-y-3">
                            <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" /> Performance Metrics
                            </h4>
                            <div className="grid grid-cols-3 gap-3">
                                {[
                                    { icon: HiOutlineShoppingBag, label: 'Total Sales', value: displayVendor.totalOrders || '0', bg: 'bg-blue-50', text: 'text-blue-600' },
                                    { icon: HiOutlineSparkles, label: 'Rating', value: displayVendor.rating || '4.5', bg: 'bg-amber-50', text: 'text-amber-600' },
                                    { icon: HiOutlineCheckBadge, label: 'Revenue', value: `₹${displayVendor.revenue || '0'}`, bg: 'bg-pink-50', text: 'text-pink-600' },
                                ].map((stat, idx) => (
                        ...
                        <div className="p-4 rounded-xl bg-indigo-50/50 border border-indigo-100 dark:bg-indigo-950/20 dark:border-indigo-900/30">
                            <h5 className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase mb-1 flex items-center gap-1">
                                <HiOutlineUserCircle /> About Store
                            </h5>
                            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed italic">
                                {displayVendor.description || "No description provided by the vendor yet."}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="p-6 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex gap-3 shrink-0">
                        ...
                        <button className={`flex-1 py-3 px-4 rounded-xl text-xs font-black transition-all shadow-md ${displayVendor.isActive
                            ...
                            {displayVendor.isActive ? 'Block Vendor' : 'Unblock Vendor'}
                        </button>
                    </div>
                </div>
            </div>
